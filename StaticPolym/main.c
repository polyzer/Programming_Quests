...
struct Polynomial *GCD(struct Polynomial *poly1, struct Polynomial *poly2)
{
	...
	if ((poly1->monomials[0].degree == 0) || (poly2->monomials[0].degree == 0)){
		printf("Степени многочленов должны быть > 0!\n");
		return NULL;
	}
...
int main () {
	...
	tempPoly1 = getPolynomialFromString("1x2-2x1+1");
	tempPoly2 = getPolynomialFromString("1x2-1");
	tempPoly3 = mulPolynomials(tempPoly1, tempPoly2);
	printPolynomial(tempPoly3);
	tempPoly3 = addPolynomials(tempPoly1, tempPoly2);
	printPolynomial(tempPoly3);
	tempPoly3 = subPolynomials(tempPoly1, tempPoly2);
	printPolynomial(tempPoly3);
